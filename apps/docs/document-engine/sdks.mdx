...
| Operation | CLI command | Description |
| --- | --- | --- |
| `doc.format.apply` | `format apply` | Apply explicit mark changes (bold, italic, underline, strike) to the target range using boolean patch semantics. |
...
| `doc.review.decide` | `review decide` | Accept or reject a tracked change (by ID or scope: all). |
