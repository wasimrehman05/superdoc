...
function PdfEditor({ document }) {
  ...
  return (
    <SuperDocEditor
      document={document} // File/Blob, URL string, or { type: 'pdf', url/data, name }
      modules={modules}
...
When using whiteboard sticker drag/drop in React, register stickers when the whiteboard instance is available and pass the sticker id via `application/sticker`.
